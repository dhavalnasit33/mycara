...
import { bulkDeleteNavbarItems, deleteNavbarItem, fetchNavbar } from "@/features/navbar/navbarThunk";


export default function Navbar() {
  const dispatch = useDispatch<AppDispatch>();
  const { navbars, total, loading } = useSelector(
  ...
  // Fetch navbar items
  useEffect(() => {
    dispatch(fetchNavbar({ page, limit, search: debouncedQuery }));
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error(err?.message || "Failed to delete navbar item.");
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await dispatch(bulkDeleteNavbarItems(selectedIds)).unwrap();
      toast.success(`${selectedIds.length} navbar items deleted successfully.`);
      setSelectedIds([]);
    } catch (err: any) {
      toast.error(err?.message || "Failed to delete navbar items.");
    }
  };
  ...
  const handleDownload = async () => {
    try {
      const result = await dispatch(
        fetchNavbar({ page: 1, limit: 1000, search: debouncedQuery })
      ).unwrap();

      const data = result.navbars.map((item: any) => ({
        Title: item.title,
        URL: item.url,
        Icon: item.icon || "-",
        Order: item.order ?? "-",
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Navbar Items");
      ...
      saveAs(blob, `navbar_${Date.now()}.xlsx`);
    } catch (err) {
  ...
  if (loading) return <div>Loading navbar items...</div>;
    ...
    <div className="space-y-8 p-6 mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage your navbar items</p>
        </div>
        <div className="flex gap-2">
          <Link to="/navbar/add">
            <Button className="flex items-center gap-2">
              <Plus className="h-4 w-4" /> Add Navbar Item
            </Button>
          </Link>
          <Button
      ...
      <Card className="shadow-sm border border-gray-200">
        <CardContent className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
          ...
          {selectedIds.length > 0 && (
            <ConfirmDialog
              title="Delete Selected Navbar Items"
              description={`This will delete ${selectedIds.length} selected navbar items.`}
      ...
      <Card className="shadow-sm border border-gray-200">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg font-semibold">
            Navbar Items <span className="text-gray-400 font-normal">({total})</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <th className="p-3 text-center w-10">
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              setSelectedIds(checked ? navbars.map((n) => n._id) : [])
            }
          />
        </th>
          ...
          <tr key={item._id} className="hover:bg-gray-50 transition-colors">
            ...
            <td className="p-3 text-center flex justify-center gap-2">
              ...
              <ConfirmDialog
                ...
                description={`This will permanently delete navbar item "${item.label}".`}
                confirmText="Delete"
