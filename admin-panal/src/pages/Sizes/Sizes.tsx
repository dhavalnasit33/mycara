...
import { fetchSizes, deleteSize, bulkDeleteSizes } from "@/features/sizes/sizesThunk";

export default function Sizes() {
  const dispatch = useDispatch<AppDispatch>();
  const { sizes, total, loading } = useSelector((state: RootState) => state.sizes);
  ...
  // Fetch sizes
  useEffect(() => {
    dispatch(fetchSizes({ page, limit, search: debouncedQuery }));
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await dispatch(bulkDeleteSizes(selectedIds)).unwrap();
      toast.success(`${selectedIds.length} sizes deleted successfully.`);
      setSelectedIds([]);
    } catch (err: any) {
      toast.error(err?.message || "Failed to delete sizes.");
    }
  };
  ...
  const handleDownload = async () => {
    try {
      const result = await dispatch(fetchSizes({ isDownload: true })).unwrap();
      const data = result.sizes.map((s: any) => ({
        Name: s.name,
      ...
      saveAs(blob, `sizes_${Date.now()}.xlsx`);
    } catch (err) {
  ...
  return (
    <div className="space-y-8 p-6 mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Sizes</h1>
        ...
        <CardContent className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
          ...
          {selectedIds.length > 0 && (
            <ConfirmDialog
              ...
              description={`This will delete ${selectedIds.length} selected sizes.`}
      ...
      <Card className="shadow-sm border border-gray-200">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg font-semibold">
            Sizes <span className="text-gray-400 font-normal">({total})</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <thead className="bg-gray-50 text-gray-700 text-sm font-medium">
                <tr>
                  <th className="p-3 w-10 text-left">
                    <Checkbox
                      checked={selectedIds.length === sizes.length && sizes.length > 0}
                  ...
                  <th className="p-3 w-32 text-left">Measurement</th>
                  ...
                  <tr key={size._id} className="hover:bg-gray-50 transition-colors">
                    <td className="p-3">
                      <Checkbox
                        ...
                        onCheckedChange={() => handleSelect(size._id)}
                      />
                    </td>
                    <td className="p-3 font-medium text-gray-900 truncate">{size.name}</td>
                    <td className="p-3">{size.measurement || "-"}</td>
                    <td className="p-3">{getStatusBadge(size.status)}</td>
                    <td className="p-3 flex justify-end gap-2">
                      ...
                      <ConfirmDialog
                        title="Delete Size"
                        description={`This will permanently delete size "${size.name}".`}
                        confirmText="Delete"
