...
const [discountValue, setDiscountValue] = useState<string>("");
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (!discountValue || Number(discountValue) <= 0) {
    toast.error(`Please enter a valid ${type === "percentage" ? "percentage" : "fixed"} value`);
    return;
  }
  if (!startDate) {
  ...
  try {
    ...
    if (createDiscount.fulfilled.match(result) || updateDiscount.fulfilled.match(result)) {
      toast.success(isEditMode ? "Discount updated successfully!" : "Discount created successfully!");
      navigate("/discounts");
    } else {
  ...
  return (
    <div className="p-6 mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{isEditMode ? "Edit Discount" : "Add New Discount"}</h1>
          <p className="text-gray-500 mt-1">{isEditMode ? "Update discount details." : "Create a new discount."}</p>
        </div>
      </div>
