...
import { Search, Filter, Plus, Edit2, Trash2, Download } from "lucide-react";
...
export default function Categories() {
  ...
  const { categories, total, loading } = useSelector(
  ...
  useEffect(() => {
    dispatch(fetchCategories({ page, limit, search: debouncedQuery }));
  }, [debouncedQuery, page, dispatch]);

  const handleDelete = async (id: string) => {
    try {
      await dispatch(deleteCategory(id)).unwrap();
      toast.success("The category has been removed successfully.");
    } catch (err: any) {
      toast.error(
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await dispatch(bulkDeleteCategories(selectedIds)).unwrap();
      toast.success(
        `${selectedIds.length} categories were removed successfully.`
      );
      setSelectedIds([]);
    } catch (err: any) {
      toast.error(
        err?.message || "Failed to delete categories. Please try again."
      );
    }
  };
  ...
  const handleDownload = async () => {
    try {
      // Fetch all categories with isDownload: true
      const result = await dispatch(
        fetchCategories({ isDownload: true })
      ).unwrap();
      ...
      const data = categories.map((cat: any) => ({
        ...
        Parent:
          typeof cat.parent_id === "object" && cat.parent_id
            ? cat.parent_id.name
            : "-",
        Status: cat.status,
      ...
      saveAs(blob, `categories_${Date.now()}.xlsx`);
    } catch (err) {
        ...
        <CardContent className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
          ...
          {selectedIds.length > 0 && (
            <ConfirmDialog
              ...
              description={`This will delete ${selectedIds.length} selected categories.`}
            ...
            <table className="w-full text-sm table-fixed">
              <thead className="bg-gray-50 text-gray-700 text-sm font-medium">
                <tr>
                  <th className="p-3 w-10 text-left">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSelectedIds(
                          checked ? categories.map((c) => c._id) : []
                        )
                      }
                    />
                  </th>
                  ...
                  <th className="p-3 w-48 text-left">Parent</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {categories.map((category) => (
                  ...
                  >
                    ...
                    <td className="p-3">{getStatusBadge(category.status)}</td>
                    <td className="p-3 flex justify-end gap-2">
                      ...
                      <ConfirmDialog
                        ...
                        description={`This will permanently delete category "${category.name}".`}
                        confirmText="Delete"
