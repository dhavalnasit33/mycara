...
export default function Brands() {
  ...
  const { brands, total, loading } = useSelector(
  ...
  // Fetch brands
  useEffect(() => {
    dispatch(fetchBrands({ page, limit, search: debouncedQuery }));
  }, [debouncedQuery, page, dispatch]);

  const handleDelete = async (id: string) => {
    try {
      await dispatch(deleteBrand(id)).unwrap();
      toast.success("The brand has been removed successfully.");
    } catch (err: any) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await dispatch(bulkDeleteBrands(selectedIds)).unwrap();
      toast.success(`${selectedIds.length} brands were removed successfully.`);
      setSelectedIds([]);
    } catch (err: any) {
      toast.error(err?.message || "Failed to delete brands. Please try again.");
    }
  };
  ...
  const handleDownload = async () => {
    try {
      const result = await dispatch(fetchBrands({ isDownload: true })).unwrap();
      const data = result.brands.map((b: any) => ({
        Name: b.name,
      ...
      saveAs(blob, `brands_${Date.now()}.xlsx`);
    } catch (err) {
        ...
        <CardContent className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
          ...
          {selectedIds.length > 0 && (
            <ConfirmDialog
              ...
              description={`This will delete ${selectedIds.length} selected brands.`}
      ...
      <Card className="shadow-sm border border-gray-200">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg font-semibold">
            Brands <span className="text-gray-400 font-normal">({total})</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <th className="p-3 w-10 text-left">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setSelectedIds(checked ? brands.map((b) => b._id) : [])
                      }
                    />
                  </th>
                  <th className="p-3 w-20 text-left">Logo</th>
                  ...
                  >
                    <td className="p-3">
                      <Checkbox
                        ...
                        onCheckedChange={() => handleSelect(brand._id)}
                      />
                    </td>
                    <td className="p-3">
                      {brand.image_url ? (
                    ...
                    <td className="p-3">{getStatusBadge(brand.status)}</td>
                    <td className="p-3 flex justify-end gap-2">
                      ...
                      <ConfirmDialog
                        title="Delete Brand"
                        description={`This will permanently delete brand "${brand.name}".`}
                        confirmText="Delete"
