...
export default function Payments() {
  ...
  const { payments, total, loading } = useSelector(
  ...
  // Fetch payments
  useEffect(() => {
    dispatch(fetchPayments({ page, limit, search: debouncedQuery }));
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await dispatch(bulkDeletePayments(selectedIds)).unwrap();
      toast.success(`${selectedIds.length} payments deleted successfully.`);
      setSelectedIds([]);
    } catch (err: any) {
  ...
  const handleDownload = async () => {
    try {
      ...
      const data = result.payments.map((p: any) => ({
        "Payment ID": p._id,
        User: p.user_id?.name,
        Plan: p.plan?.name || "-",
      ...
      saveAs(blob, `payments_${Date.now()}.xlsx`);
    } catch (err) {
        ...
        <CardContent className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
          ...
          {selectedIds.length > 0 && (
            <ConfirmDialog
              ...
              description={`This will delete ${selectedIds.length} selected payments.`}
...
<Card className="shadow-sm border border-gray-200 rounded-lg">
  <CardHeader className="pb-2">
    <CardTitle className="text-lg font-semibold">
      Payments <span className="text-gray-400 font-normal">({total})</span>
    </CardTitle>
  </CardHeader>
  <CardContent>
            ...
            <th className="p-3 w-10 text-left">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setSelectedIds(checked ? payments.map((p) => p._id) : [])
                }
              />
            </th>
            <th className="p-3 w-40 text-left">Transaction ID</th>
            <th className="p-3 w-32 text-left">User</th>
            <th className="p-3 w-32 text-left">Payment Method</th>
            <th className="p-3 w-24 text-left">Discount</th>
            <th className="p-3 w-24 text-left">Amount</th>
            ...
            <tr key={payment._id} className="hover:bg-gray-50 transition-colors">
              <td className="p-3">
                <Checkbox
                  ...
                  onCheckedChange={() => handleSelect(payment._id)}
                />
              </td>
              <td className="p-3 font-medium text-gray-900 truncate">{payment.transaction_id}</td>
              <td className="p-3">{payment.user_id?.name}</td>
              <td className="p-3">{payment.payment_method}</td>
              <td className="p-3">${payment?.discount_amount?.toFixed(2)}</td>
              <td className="p-3">${payment?.amount_paid?.toFixed(2)}</td>
              ...
              <td className="p-3 flex justify-end gap-2">
                <ConfirmDialog
                  ...
                  description={`This will permanently delete payment "${payment._id}".`}
                  confirmText="Delete"
