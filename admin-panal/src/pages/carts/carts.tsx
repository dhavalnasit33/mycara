...
export default function CartPage() {
  ...
  const { carts, total, loading } = useSelector((state: RootState) => state.carts);
  ...
  useEffect(() => {
    dispatch(fetchCartItems({ page, limit, search: searchQuery }));
      ...
      <Card className="shadow-sm border border-gray-200">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg font-semibold">
            Cart Items <span className="text-gray-400 font-normal">({total})</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <thead className="bg-gray-50 text-gray-700 text-sm font-medium">
                <tr>
                  <th className="p-3 w-10 text-left">
                    <Checkbox
                      checked={selectedIds.length === carts.reduce((a, c) => a + c.items.length, 0)}
                      onCheckedChange={(checked) => {
                        if (checked) {
                          const allIds: string[] = [];
                          carts.forEach((cart) =>
                            cart.items.forEach((item) => allIds.push(item._id))
                          );
                  ...
                  cart.items.length > 0 ? (
                    cart.items.map((item) => (
                      <tr key={item._id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="p-3">{cart.user_id?.name || "N/A"}</td>
                        <td className="p-3">{cart.user_id?.email || "N/A"}</td>
                    ...
                    <tr key={cart._id} className="border-t hover:bg-gray-50">
